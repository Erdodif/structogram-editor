...
export function statementToJSX(
    statement: Statement,
    mapping: number[],
    ref: RefObject<StructogramController>,
    ...
    switch (statement.type()) {
        ...
        case StatementType.S_IF://TODO
            ...
            if (sIf.statementBlocks.length > 0) {
                for (let i = 0; i < sIf.statementBlocks[0].length; i++) {
                    truePart.push(statementToJSX(sIf.statementBlocks[0][i], [...mapping, 0, i], ref, updateControllerState));
                }
            }
            let falsePart: JSX.Element[] = [];
            if (sIf.statementBlocks.length > 1) {
                for (let i = 0; i < sIf.statementBlocks[1].length; i++) {
                    falsePart.push(statementToJSX(sIf.statementBlocks[1][i], [...mapping, 1, i], ref, updateControllerState));
                }
            }
            return <RIfStatement
                key={mappingToId(mapping)}
                mapping={mapping}
                content={sIf.content}
                statementBlocks={[truePart, falsePart]}
                controller={ref}
                updateControllerState={updateControllerState} />
        case StatementType.S_SWITCH://TODO
            ...
            for (let i = 0; i < sSwitch.blocks.length; i++) {
                ...
                for (let j = 0; j < sSwitch.blocks[i].statements.length; j++) {
                    statements.push(statementToJSX(sSwitch.blocks[i].statements[j], [...mapping, i, j], ref, updateControllerState))
                }
        ...
        case StatementType.S_LOOP:
            ...
            for (let i = 0; i < sLoop.statements.length; i++) {
                ...
                statements.push(statementToJSX(sLoop.statements[i], subMap, ref, updateControllerState));
            }
            return <RLoopStatement
                key={mappingToId(mapping)}
                mapping={mapping}
                content={statement.content}
        ...
        case StatementType.S_LOOP_REVERSE:
            ...
            for (const index in sRLoop.statements) {
                ...
                statements.push(statementToJSX(sRLoop.statements[index], subMap, ref, updateControllerState));
            }
            return <RReversedLoopStatement
                key={mappingToId(mapping)}
                mapping={mapping}
                content={statement.content}
                statements={statements}
